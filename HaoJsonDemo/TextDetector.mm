...
+(UIImage *)detectTextRegions:(UIImage *)orgImg{
    ...
    if (!groups.empty()) {
        ...
        result = [UIImage imageWithCVMat:orgMat];
...
+(NSArray *)UIImagesOfTextRegions:(UIImage *)orgImg withLocations:(NSMutableArray *)locations{
    ...
    if (!groups.empty()) {
        ...
        cout<<"Sorting and Merging groups...."<<endl;
        [self sortAndMergeGroups:groups andResult:results];
        ...
        for (int i = 0; i < len ; i++){
            ...
            //[imgArray addObject:[UIImage imageWithCVMat:orgMat(results.at(i))]];
            NSString *imgfile = [NSString stringWithFormat:@"%d.jpg",i];
            imwrite([self filePathWithFileName:imgfile], orgMat(results.at(i)));
            
            CGRect cgrect = CGRectMake(results.at(i).x, results.at(i).y, results.at(i).width, results.at(i).height);
            [locations addObject:[NSValue valueWithCGRect:cgrect]];
